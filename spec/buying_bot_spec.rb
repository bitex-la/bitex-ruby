...
describe Bitex::JsonApi::BuyingBot do
  ...
  describe '.all' do
    ...
    context 'with unauthorized key', vcr: { cassette_name: 'buying_bots/all/unauthorized' } do
    ...
    context 'with any level key' do
      ...
      context 'enough funds', vcr: { cassette_name: 'buying_bots/all/authorized' } do
  ...
  describe '.create' do
    subject { client.buying_bots.create(amount: amount, orderbook_id: orderbook_id) }

    let(:amount) { 100 }
    let(:orderbook_id) { '1' }

    context 'with unauthorized key', vcr: { cassette_name: 'buying_bots/create/unauthorized' } do
      it_behaves_like 'Not enough permissions'
    end

    context 'with unauthorized level key', vcr: { cassette_name: 'buying_bots/create/unauthorized_key' } do
      it_behaves_like 'Not enough level permissions'
    end

    context 'with authorized level key', vcr: { cassette_name: 'buying_bots/create/authorized' } do
  ...
  describe '.find' do
    subject { client.buying_bots.find(id: id) }

    let(:id) { '1' }

    context 'with unauthorized key', vcr: { cassette_name: 'buying_bots/find/unauthorized' } do
  ...
  describe '.cancel!' do
    ...
    context 'with unauthorized key', vcr: { cassette_name: 'buying_bots/cancel/unauthorized' } do
      it_behaves_like 'Not enough permissions'
    end

    context 'with unauthorized level key', vcr: { cassette_name: 'buying_bots/cancel/unauthorized_key' } do
      it_behaves_like 'Not enough level permissions'
    end
